...
- INTERFACE_PATH (optional): e.g., `src/interfaces/IVault.sol`
- DESIGN_DOC_PATH (optional): e.g., `docs/designs/2025-01-15-vault-design.md`

If INTERFACE_PATH or DESIGN_DOC_PATH are not provided, continue the audit but note in
the report that cross-reference checks were skipped.
...
- Solidity compiler version:
  ...
  - Check current latest stable (0.8.x); recommend updating if more than 2 minor versions behind
...
5. **Never apply changes that increase contract size** — if applying an "optimization"
