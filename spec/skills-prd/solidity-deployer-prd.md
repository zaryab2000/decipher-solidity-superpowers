# Skill PRD: `solidity-deployer`

**Output file:** `skills/solidity-deployer/SKILL.md`
**Supporting file:** `skills/solidity-deployer/deployment-checklist.md`

**When:** Before any deployment to any network — testnet, mainnet, or any public fork.
...
Deployment mistakes are irreversible. An unverified contract cannot be trusted or audited.
A contract deployed with the deployer EOA as owner — instead of a multisig — is a single
private key compromise away from catastrophic loss. A deploy script that doesn't run
deterministically across environments produces different results on testnet vs mainnet.

This skill turns deployment from an ad-hoc "forge create" into a reproducible, auditable,
versioned procedure with a written record.
...
## Pre-Deployment Checklist (Mandatory — All Items Must Pass)

The SKILL.md must reproduce this checklist as a table. The coding agent must execute each
...
The SKILL.md must include these as a copy-paste reference:
...
The SKILL.md must include post-deployment checks using `cast`:

```bash
# Verify contract source on Etherscan
# (done automatically with --verify in the script, but confirm it's showing)
...
Post-deployment verification table for the SKILL.md:
...
The manifest is written by the deploy script. Format:
...
## Supporting File: deployment-checklist.md

This file lives at `skills/solidity-deployer/deployment-checklist.md`.

Required content:
- Full pre-deployment checklist with remediation for each item
- Full post-deployment checklist with `cast` commands
- Network-specific config templates (mainnet.json, sepolia.json, arbitrum.json)
- Common deployment mistakes and how to detect them (uninitialized proxy, wrong owner, wrong constructor args)
- Environment variables reference (what must be set in `.env` before deploying)
- Multi-sig setup guidance (Gnosis Safe threshold recommendations)
