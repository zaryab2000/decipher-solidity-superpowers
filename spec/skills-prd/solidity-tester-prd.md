# Skill PRD: `solidity-tester`

**Output file:** `skills/solidity-tester/SKILL.md`
**Supporting file:** `skills/solidity-tester/invariant-testing-guide.md`

**When:** During or after the build phase, when the contract handles value (ETH, tokens, shares,
debt positions) and needs testing beyond unit tests. Also when adding property-based tests or
fork tests to existing contracts.

---

## Why This Skill Exists

Unit tests verify that individual functions behave correctly when called with specific inputs.
They do not verify that the system stays consistent across arbitrary sequences of operations.

Invariant violations — the entire class of bugs that unit tests miss — are what cause the
largest DeFi exploits: share inflation attacks, insolvency through rounding, reentrancy across
function combinations, and accounting corruption from unexpected call sequences.

Foundry's fuzz and invariant testing framework finds these bugs automatically. The skill
enforces that this framework is used on every contract that handles value.
...
The coding agent must create tests in this exact directory layout:
...
### Rules for Writing Fuzz Tests

**Rule 1: Always use `bound()` to constrain inputs**

Unbounded fuzz inputs produce many wasted runs that always revert on invalid values. Constrain
to the meaningful input range:
...
function testFuzz_deposit_updatesBalance(uint256 amount) public {
    // Bound to: > 0 (non-trivial), < uint128.max (no overflow when multiplied)
...
Fuzz tests assert relationships that must hold, not specific return values:
...
function testFuzz_withdrawAll_leavesNoAssets(uint256 depositAmount) public {
    depositAmount = bound(depositAmount, 1e6, type(uint128).max); // min to avoid dust
...
The SKILL.md must direct the coding agent to add this to `foundry.toml`:
...
```solidity
// Tests that two users don't interfere with each other
function testFuzz_deposit_twoUsersAreIsolated(
    ...
    // Alice's shares should not change after Bob's deposit
...
contract VaultHandler is Test {
    ...
    // They are compared against actual state in invariant assertions
    ...
    uint256 public ghost_zeroDepositCount; // track skipped zero-amount calls
    ...
    constructor(Vault _vault, IERC20 _token) {
        ...
        // Create a realistic actor pool — more actors = better coverage of multi-user scenarios
    ...
    function deposit(uint256 amount, uint256 actorIndexSeed) external useActor(actorIndexSeed) {
        ...
        // Setup: give actor enough tokens
    ...
    function withdraw(uint256 amount, uint256 actorIndexSeed) external useActor(actorIndexSeed) {
        ...
        if (maxWithdraw == 0) {
            ...
            return; // Skip: actor has nothing to withdraw
        }
...
contract VaultInvariantTest is Test {
    ...
    function setUp() public {
        ...
        // Optional: exclude specific senders to prevent admin key conflicts
    ...
    /// Violation: vault is insolvent (owes more than it holds)
    ...
    /// Violation: shares were minted or burned without corresponding asset movement
    ...
    /// @notice No user can have more shares than totalSupply
    /// Violation: share accounting overflow or double-counting
    function invariant_noUserSharesExceedTotalSupply() public view {
        address[] memory actors = handler.getActors(); // add getActors() to handler
        uint256 sumOfShares;
        for (uint256 i; i < actors.length; ++i) {
            sumOfShares += vault.balanceOf(actors[i]);
        }
        assertEq(
            sumOfShares,
            vault.totalSupply(),
    ...
    /// @notice totalAssets must never decrease when only deposits occur
    function invariant_totalAssetsMonotoneOnDeposit() public view {
        // This invariant is checked implicitly through ghost_totalDeposited
        assertGe(
            vault.totalAssets(),
            0,
            "INVARIANT VIOLATED: totalAssets is negative (impossible, but catches underflow)"
        );
    }

    // ── Summary (runs after every call sequence for debugging) ────────────────
...
show_metrics = true      # shows call distribution — use to verify handler is exercising all paths
```
...
The SKILL.md must explain each category with examples:

### Accounting Invariants (Most Critical)
Verify that assets and shares add up correctly.
...
### Conservation Invariants
Verify that value is not created or destroyed.
...
Verify counters that should only go in one direction.
...
Verify that privileged operations cannot be performed by unauthorized callers.

```
// Pause state
paused == true → deposit() reverts
paused == true → withdraw() reverts
paused == false → deposit() succeeds for valid inputs
...
### Safety Invariants
Verify value bounds and sanity conditions.
...
contract VaultForkTest is Test {
    ...
    function test_fork_chainlinkPrice_notStale() public {
        // Verify oracle freshness check works on real data
        (, int256 price, , uint256 updatedAt, ) =
            AggregatorV3Interface(CHAINLINK_ETH_USD).latestRoundData();

        assertGt(price, 0, "Fork: ETH price should be positive");
        assertGt(updatedAt, block.timestamp - 1 hours, "Fork: ETH price should not be stale");
    }
}
```
...
1. **Always pin to a specific block number.** Floating `vm.createFork("mainnet")` means tests
   change as the chain advances. Pin the block.
...
3. **Cache RPC responses.** After the first run, forge caches responses. CI is fast after warmup.

4. **Do not fuzz over forked state.** Fuzz inputs inside a fork test trigger repeated RPC calls
   and hit rate limits. Use parameterized fork tests with specific values only.

5. **Test with real whale balances.** Etherscan → search the token → top holders → use one.
   `vm.prank(whale)` + `vm.deal()` or direct `deal()` for ERC-20.

---

## Supporting File: invariant-testing-guide.md

This file lives at `skills/solidity-tester/invariant-testing-guide.md` and is loaded on demand.
...
Extended handler with `getActors()` helper, `warp` handler for time-based invariants, and
`donate` handler to test donation attacks.

**Section 2: Ghost Variable Patterns by Contract Type**

For ERC-4626 vaults:
```
ghost_totalDeposited    ← sum of all deposit() assets
ghost_totalWithdrawn    ← sum of all withdraw() assets
ghost_totalFeesPaid     ← sum of fees collected
ghost_maxDepositor      ← address of largest depositor
```
...
ghost_transferCount     ← number of successful transfers
```
...
ghost_totalLiquidated   ← sum of all liquidation() amounts
```

**Section 3: Common Invariant Formulas by Contract Type**

ERC-20: `totalSupply == ghost_totalMinted - ghost_totalBurned`
ERC-4626: `totalAssets == ghost_totalDeposited - ghost_totalWithdrawn`
AMM: `k == reserve0 * reserve1` (constant product)
Lending: `utilization == totalBorrowed / (totalBorrowed + totalLiquidity)`

**Section 4: Debugging Failed Invariant Sequences**

When an invariant fails, Foundry prints the call sequence that caused it. The guide must explain:
- How to read the call sequence output
- How to reproduce the failure deterministically with `--ffi`
- Common root causes: off-by-one in ghost tracking, missing handler function, incorrect bounds

**Section 5: Call Summary Analysis**

The `invariant_callSummary` function logs call distribution. The guide must explain:
- If `deposit` is called 95% of the time and `withdraw` 5%, the invariant run is biased
- Fix by adding weight hints or adjusting handler function ratios
- `show_metrics = true` in foundry.toml reveals the distribution
